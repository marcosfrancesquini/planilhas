    ...
    with pd.ExcelWriter(saida_xlsx, engine="xlsxwriter") as writer:
        for K in range(2, N+1):
            ...
            for row in range(first_row, last_row + 1):
                # Fórmulas (mantidas) + valores em cache (para Linux / openpyxl)
                formula_k = f'=IF(I{row}>={max_x},E{row},"")'
                formula_l = f'=IF(J{row}<={min_x},E{row},"")'
                i = row - 2  # índice do DataFrame
                numero = df.iloc[i]["Número"]
                maxv = df.iloc[i]["Maximo de vezes"]
                minv = df.iloc[i]["Minimo de vezes"]
                res_k = numero if maxv >= max_x else ""
                res_l = numero if minv <= min_x else ""
                ws.write_formula(row - 1, 10, formula_k, None, res_k)  # K
                ws.write_formula(row - 1, 11, formula_l, None, res_l)  # L
